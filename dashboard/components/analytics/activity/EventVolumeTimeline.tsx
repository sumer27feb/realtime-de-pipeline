...
interface EventVolumeTimelineProps {
  data: EventActivity[];
}
...
export function EventVolumeTimeline({ data }: EventVolumeTimelineProps) {
  ...
  const types = EVENT_TYPES.filter((type) => eventTypes.includes(type));

  const getCount = (windowStart: string, eventType: string) => {
    const row = data.find(
      (item) =>
  ...
  const option = {
    ...
    series: types.map((eventType) => ({
      ...
      data: windows.map((windowStart) => getCount(windowStart, eventType)),
    })),
  };
  ...
  return (
    <section className="border border-white/10 bg-[#0b1018]">
      ...
      <div className="h-[360px] w-full px-3 pb-4 pt-3 sm:h-[400px] sm:px-5">
        {data.length > 0 ? (
          <ReactECharts
            option={option}
            style={{
